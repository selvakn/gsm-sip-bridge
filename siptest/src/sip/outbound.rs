...
pub fn place_call(
    ...
    let resp1 = wait_final_response(socket, ring_timeout)?;
    ...
    loop {
        ...
        let Some(resp) = socket.recv_response(RESPONSE_POLL)? else {
            continue;
        };
        match resp.status {
            100 => continue,
...
fn wait_final_response(
    ...
    loop {
        ...
        if let Some(resp) = socket.recv_response(RESPONSE_POLL)? {
            if resp.status >= 200 {
                return Ok(Some(resp));
            }
