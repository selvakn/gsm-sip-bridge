...
# extract_latest_pcscf, render_line_strongswan_conf, render_line_swanctl_conf,
# and render_line_updown_script live in docker/lib/render_helpers.sh
# (specs/021-entrypoint-supervise-rust Phase 0 — extracted with unchanged
# logic and bats-core coverage before their Phase 1 port to Rust).
# shellcheck source=docker/lib/render_helpers.sh
source "$(dirname "${BASH_SOURCE[0]}")/lib/render_helpers.sh"
...
if [ "$TUNNEL_ENGINE" = "strongswan" ]; then
    ...
    # shellcheck disable=SC2153 # VPCD_PORT comes from `eval "$SHELL_ENV"` above
