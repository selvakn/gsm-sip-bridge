...
| `register_request_uri` | enum | `pcscf` | What goes in the REGISTER request line: `pcscf` (the literal P-CSCF address) or `home-domain` (the `ims.mnc<MNC>.mcc<MCC>.3gppnetwork.org` realm that TS 24.229 §5.1.1.2 mandates). Carriers disagree and each form is rejected outright somewhere, so this has no universally correct value. **Jio needs `home-domain`** — its P-CSCF finds its own address in the request line, routes the REGISTER to itself and answers `483 Too Many Hops` (or a blanket `403 Forbidden`) before any challenge, which reads exactly like an unprovisioned subscriber. **Airtel needs the `pcscf` default** — it answers `406 User Unknown` to the realm form |
