...
| `sms_delivery_report` | boolean | `true` | Same meaning and default as `[vowifi]`'s field above — SMS-over-IP's RP-layer acknowledgement is the same TS 24.341 procedure regardless of access. Kept as its own key rather than shared from `[vowifi]` so a VoLTE-only deployment (no `[vowifi]` section) has a switch of its own, and so the two accesses can be set independently if a carrier is ever found that needs them to differ |
| `respect_caller_privacy` | boolean | `true` | Same meaning and default as `[vowifi]`'s field above — the RFC 3325 `Privacy` header is a SIP/IMS-layer signal, not an access one. Kept as its own key for the same reason as `sms_delivery_report` above |
