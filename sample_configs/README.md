...
| [`vowifi-only-cs-disabled.toml`](vowifi-only-cs-disabled.toml) | Same as above, but with `[cs].enabled = false` — no plain circuit-switched calls at all, so the circuit-switched daemon does no background modem probing. See `docs/configuration.md#cs` and `specs/026-disable-circuit-switched`. |
