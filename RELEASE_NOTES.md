...
- **Inbound VoLTE calls on Jio now connect instead of ringing forever on the caller's side.** The LTE path always answered network-initiated requests on the wrong Gm leg for Jio — the same quirk `[vowifi].respond_on_client` already fixed, now ported to VoLTE as `[volte].respond_on_client` (default `true`, confirmed required on Jio and harmless on Vodafone). Also added `[volte].register_request_uri`, now defaulting to `"home-domain"` like `[vowifi]` (confirmed working on Jio and Vodafone), for Jio's identical P-CSCF loop-detection rejection of the REGISTER Request-URI's address form. Live-verified end-to-end on a real Jio line.

- **VoLTE now works on carriers whose IMS PDN comes up IPv4-only** (confirmed: Jio) **instead of only the IPv6 this bridge previously assumed.** `attach()` now also configures the host interface via DHCP when the PDN hands out an IPv4 address, alongside the existing IPv6 path, so a dual-stack PDN gets both and teardown releases whichever family was actually used. Live-verified on Jio (IPv4-only) and Vodafone (IPv6-only).
