...
- **Inbound Jio VoWiFi calls still failed after the `Supported` header fix below: most rang, got answered, and were torn down by the network within milliseconds of the `ACK` with `BYE Reason:SIP;cause=503;text="IO: SIP SDP Protocol Error."`** The teardown was not about the SDP body at all — Jio's Alcatel-Lucent border elements validate the method set this bridge advertises in the `Allow` header of its `200 OK`, and reject an answer that does not claim `UPDATE`. Only calls routed through one of those elements were affected, which made the failure look carrier-random and led an earlier investigation to conclude, wrongly, that nothing on this side could fix it. Bisected on a live Jio line down to that single method: 0 of 12 such calls connected without `UPDATE`, 12 of 12 connected with it, and substituting `PRACK` for `UPDATE` (the same number of methods) failed again. Responses to an inbound INVITE now advertise `UPDATE`; every other response, including the `405` this UAS sends for a method it genuinely does not serve, still states only what it really implements. Jio never sends an `UPDATE` — it checks the claim without ever using it. See `docs/jio-lucent-sbc-sdp-protocol-error.md`.
...
- **Inbound VoLTE calls on Jio rang and looked answered on this bridge's own side, but never connected on Jio's: the caller kept hearing ringback until Jio's own timeout tore the call down.** The host-side LTE path installs its own Gm IPsec SAs exactly as the VoWiFi/ePDG path does, but always answered network-initiated requests on the socket a request arrived on rather than the client leg Jio actually reads responses from — the same quirk `[vowifi].respond_on_client` already existed to work around, never ported to VoLTE. A new `[volte].respond_on_client` fixes it, defaulting to `true` (confirmed required on Jio, live-verified harmless on Vodafone); existing deployments needing the old behaviour can set it `false`. Also added `[volte].register_request_uri` (default `"pcscf"`, unchanged from today) for the identical Jio P-CSCF loop-detection rejection of the address-form REGISTER Request-URI that `[vowifi].register_request_uri` already fixes; set `"home-domain"` for a carrier known to need it. Live-verified end-to-end on a real Jio line.

- **VoLTE could attach an IMS PDN and then leave it completely unrouted, with no visible error beyond the usual "not routable" warning, on a carrier whose PDN comes up IPv4-only instead of the IPv6 this bridge had always assumed (confirmed: Jio; the same modem gave Vodafone an IPv6-only bearer).** `attach()` only ever configured the host interface for an IPv6 PDN address. It now also runs a one-shot DHCP request against the carrier's own DHCP relay when the PDN hands out an IPv4 address, alongside (not instead of) the existing IPv6 path, so a dual-stack PDN gets both; teardown releases whichever family was actually configured. Live-verified on Jio (IPv4-only) and Vodafone (IPv6-only).
...
- **A SIP client reachable only over a routed network (Tailscale, or any VPN/mesh) got complete silence in both directions on every call, even though registration, ringing, and answering all worked normally.** Neither the SIP transport nor any account's RTP media config ever set PJSIP's `public_addr`, so `create_rtp_rtcp_sock()` fell back to `pj_gethostip()`'s default-route candidate — a private, host-internal address under `network_mode: host` — for both the SDP `c=` line and the Contact/Via headers. A remote softphone was told to send its RTP to an address it couldn't route to, so its outbound audio never arrived; with no inbound RTP, PJSIP's symmetric-RTP latching never triggered either. Reported in [#77](https://github.com/selvakn/gsm-sip-bridge/issues/77). A new optional `[sip].public_addr` setting (an IPv4 literal or hostname, resolved once at startup, never in the call path) now flows into the SIP transport and every PJSIP account-config rebuild this bridge performs — including `Account::set_identity`'s inbound caller-ID rewrite in SIP-server mode, which previously reset any address not re-applied there on every single inbound call. IPv6 is rejected at config load rather than silently mismatched, since this bridge's SIP transport is IPv4-only. Deployments that leave the setting unset are unaffected. Verified against real linked PJSIP (SDP/Contact carry the configured address, including across the `set_identity` rewrite); not yet exercised against a live Tailscale-reachable softphone — see `specs/050-sip-public-addr/quickstart.md`.

## v8.16.1

- **A line whose SIM drops off the modem bus mid-run is now recovered automatically, instead of staying dead until a human restarts it.** The supervisor has always known the remedy for a dropped USIM (a soft radio cycle, `AT+CFUN=0` -> `AT+CFUN=1`), but it is driven by agent *exits* — and only the startup path ever produced one. A SIM that was already gone at boot was therefore recovered, while a SIM present at startup and lost later was not: the renewal path logged the same `AT+CSIM failed` and retried on backoff indefinitely, invisible to the supervisor. Observed on a live line on 2026-09-11, where the SIM dropped at 16:59 and the line stayed unreachable for 10h12m across 309 identical failed renewals, with the remedy about three minutes away throughout. A renewal that keeps failing now gives up after six consecutive attempts — the whole existing backoff ladder, about 155 seconds, so an ordinary carrier or attachment blip is still ridden out in place — and exits so the supervisor can apply it. A call in progress still defers renewal exactly as before, so no live call is cut short by this. No configuration change.
- **A dead Gm signalling connection no longer floods the log while its re-registration is already pending.** The liveness probe re-escalated on every one-second idle tick for as long as the fault lasted, because its attempt counter is cleared only by a *successful* renewal. The same incident above logged 36,588 escalations of a re-registration that was already queued — roughly 36 MB of log a day for one fault — and reported an attempt count in the tens of thousands in `vowifi-status`. The probe now stands down once an escalation is pending, since the re-registration replaces that transport wholesale anyway.
