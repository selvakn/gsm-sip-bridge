...
- **Inbound VoLTE calls on Jio now connect instead of ringing forever on the caller's side.** The LTE path always answered network-initiated requests on the wrong Gm leg for Jio — the same quirk `[vowifi].respond_on_client` already fixed, now ported to VoLTE as `[volte].respond_on_client` (default `true`, confirmed required on Jio and harmless on Vodafone). Also added `[volte].register_request_uri` (default unchanged, `"pcscf"`) for Jio's identical P-CSCF loop-detection rejection of the REGISTER Request-URI's address form. Live-verified end-to-end on a real Jio line.
