...
- **A modem that stops answering can no longer take a line off the air silently.** On 2026-08-16 a line was unreachable for **2 hours 45 minutes** — callers heard "not reachable / switched off" — while the container reported `healthy`, Prometheus reported the line registered, and `vowifi-status` answered normally. A routine hourly re-registration had issued a SIM command and blocked forever in `read(2)` on the modem's serial port, on the very thread that also answers incoming calls; the registration then quietly lapsed. Nothing noticed, and nothing recovered it, because the supervisor only restarts a line whose process has *exited* and this one was alive, just permanently parked in a syscall. Four things change. Every modem command is now bounded by a deadline the caller controls, so an unresponsive modem produces an error the surrounding retry logic already handles instead of freezing the line. A watchdog detects a line that has stopped making progress and restarts it, covering the background message sweep as well as the main loop — that sweep touches the modem every 20 seconds and was previously unwatched, and a sweep that wedges also blocks the next re-registration behind it. Registration expiry is now visible in `vowifi-status` (`expires_in: -9752s (LAPSED)`), in a new `gsm_sip_bridge_vowifi_registration_expires_in_seconds` metric, and in the container healthcheck, so a lapsed line can no longer report as healthy anywhere. And renewal now follows the lifetime the network actually granted rather than assuming an hour. Repeated failures escalate through the existing SIM-recovery ladder and alert once, then retry slowly so a line recovers by itself if the hardware does. No configuration change; `[vowifi].watchdog_recovery_enabled = false` preserves a wedged line for diagnosis without hiding it. See specs/039-at-stall-watchdog.
