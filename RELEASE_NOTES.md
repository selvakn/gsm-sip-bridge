...
- **Restarting the container no longer costs every line ~2.5 minutes of silence.** A graceful stop used to only signal every process and remove each line's namespace *name* — never the tunnel interface itself, which is the only thing that actually releases the XFRM `if_id` a line's `tun23-N` device claims. The next start would then find that `if_id` still held by a leftover interface inside a namespace nothing could address, and fail to recreate it until the kernel reaped the old container's mount namespace on its own. Stop now waits for every child, terminates each line's IKE_SA, flushes this deployment's own XFRM state, and explicitly deletes each line's tunnel interface and veth pair before its namespace — in that order, bounded, with a fallback that still reaches the releasing steps if the stop allowance runs low. A force-killed run's leftovers are now reclaimed on the next start too, via a namespace directory shared with the host (`docker-compose.yml`'s new `/var/run/netns` bind mount and `stop_grace_period: 60s`). VoLTE lines get the same veth cleanup, which they never had either. No configuration change. See specs/041-shutdown-resource-cleanup.
