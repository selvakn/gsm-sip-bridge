...
- The retry window is a bounded, on-the-order-of-minutes duration; its exact duration is a planning-phase decision, not fixed by this spec. **Implementation refinement**: the window governs only *when the failure alert fires* (sized to absorb ordinary USB/device enumeration delay, the observed real-world case) — it does not force the system to stop watching altogether afterward. Continuing to re-check a still-missing line costs one lightweight `discover` re-run per poll interval, which is cheap enough to keep doing for the life of the process specifically so a line can still self-heal (with a paired recovery notice) even after being declared failed, per the edge case below. This only applies while nothing has started yet — once any configured line is actually running, watching stops entirely per the implementation-discovered limitation noted above.
...
- **Implementation-discovered limitation** (found during `/speckit-implement`, not covered by the clarification session above): the retry-to-running behavior in FR-004 applies when *no* VoWiFi line resolved on the first discovery pass. If at least one configured line already resolved and started, a *different* still-missing configured line does not self-heal without a container restart — the shared tunnel daemon's P-CSCF plugin only reads its enabled-connections list at its own process start, and reloading that list would require restarting the daemon, which would drop every other line's already-established calls to fix the one that was broken. This case is still fully covered by FR-001/FR-006/FR-007/FR-008/FR-009 (immediate detection, status/health visibility, and alerting all still apply with no retry window) — it just doesn't self-heal live. Detailed in `research.md` R3.
