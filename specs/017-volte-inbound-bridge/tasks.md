...
**Status**: Generated before implementation. Every task unchecked — verified
...
- [ ] T007 [P] [US5] Create `gsm-sip-bridge/src/volte/sms.rs` and register it in `gsm-sip-bridge/src/volte/mod.rs`
- [ ] T008 [US5] Define `InboundMessage` with its delivery route in `gsm-sip-bridge/src/volte/sms.rs`
- [ ] T009 [US5] Implement the deduplication key so a retransmission is recognised (FR-027, FR-037) in `gsm-sip-bridge/src/volte/sms.rs`
- [ ] T010 [US5] Handle messages arriving over the registration, reusing the existing acknowledgement (FR-025) in `gsm-sip-bridge/src/volte/sms.rs`
- [ ] T011 [US5] Handle messages the network leaves in modem storage, reusing `gsm-sip-bridge/src/sms/reader.rs` (FR-036)
- [ ] T012 [US5] Recover messages already in modem storage at startup (US5 scenario 7) in `gsm-sip-bridge/src/volte/sms.rs`
...
- [ ] T016 [P] [US5] Create `gsm-sip-bridge/tests/test_volte_sms.rs`: a message on each route is recorded and forwarded once
- [ ] T017 [P] [US5] Test the same message arriving on **both** routes is recorded once (FR-037) in `gsm-sip-bridge/tests/test_volte_sms.rs`
- [ ] T018 [P] [US5] Test a retransmission is acknowledged but not duplicated (FR-027) in `gsm-sip-bridge/tests/test_volte_sms.rs`
- [ ] T019 [US5] **Test that a crash between acknowledging and recording cannot lose a message** — ordering is the whole safety property, in `gsm-sip-bridge/tests/test_volte_sms.rs`
