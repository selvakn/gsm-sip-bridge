...
- [X] T019 `/code-review medium` on the branch surfaced two real bugs in T007/T008's implementation, both fixed with a regression test added for the more serious one:
  1. **Stale veth-listener reuse (high severity)**: the early-spawned veth listener's `VETH_INVITE_TIMEOUT` clock started at the provisional, not at the real `200 OK` — on a carrier with a long early-media window (Jio: ~13.7s) it could already have timed out by the time `200 OK` arrived, and reusing that stale failed receiver unconditionally would hang up a carrier leg the carrier had just successfully answered (violates FR-006). Fixed: `on_carrier_response` now does a non-blocking `try_recv()` on the reused receiver first and falls back to a fresh `spawn_veth_uas_listener` call when it already resolved to a failure. New test: `stale_early_veth_failure_falls_back_to_a_fresh_listener_at_200_ok` — verified it actually catches the regression by reverting the fix and confirming the test fails, before restoring it.
  2. **Stale RTP address on a genuinely different final SDP (medium severity)**: skipping the `200 OK`'s RTP reconnect whenever early media had already connected the socket assumed the early SDP's media address always matches the final answer's — true only for the `Require: 100rel` case, not for early media in general (FR-008 deliberately doesn't require the header). Fixed by removing the skip: `UdpSocket::connect` to the *same* address is a no-op-cost kernel call, so reconnecting unconditionally (as it worked before this feature) is both simpler and correct. This also removed `early_media_rtp_connected` as a field — it had no remaining reader and was fully redundant with `early_veth_rx.is_some()`.

  A third finding (redundant `sdp::parse_answer` call across early-media time and `200 OK` time) was left as-is — genuinely minor (one cheap string scan per outbound call), and threading a cached parse result through would add a field for marginal benefit.
