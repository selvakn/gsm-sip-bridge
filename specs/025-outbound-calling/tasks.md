...
- [~] T023 [US1] **Hardware verification, end to end, 2026-08-03 — PARTIAL, real signaling+dial confirmed, audio bridging blocked by this unit's environment**: see the combined T023/T033 writeup below T036.

**Checkpoint**: PBX-originated outbound calling's signaling and dial dispatch
are confirmed against real circuit-switched hardware (T023, 2026-08-03).
Full audio-bridged verification remains open, blocked by this session's test
unit's ALSA enumeration rather than by this codebase — see T023's writeup.
...
- [~] T033 [US3] **Hardware verification, 2026-08-03 — PARTIAL, real signaling+dial confirmed, audio bridging blocked by this unit's environment**:
  ...
  **Once unblocked**: `CardPool` discovered the modem cleanly (`/dev/ttyUSB2` or `/dev/ttyUSB3` depending on the run — `probe_at_port` correctly found the real AT-capable port each time, avoiding the bad `ttyUSB1` once it no longer existed as a device), assigned slot 0, `Ready`, `has_audio_capability=true`, IMEI/4G-LTE read back correctly. A minimal from-scratch SIP test client (`REGISTER` with real MD5 digest auth against account `1001`, then `INVITE +919789063708`) was run against the live registrar:
  ...
  2. The INVITE got a real `302 Moved Temporarily` (`sip_server: redirecting a registered phone's dial-out attempt destination=+919789063708 contact=sip:+919789063708@gsm-sip-bridge:5062`) — **T033's registrar-redirect mechanism confirmed working on real hardware**, not just in the unit-test suite.
  3. Following the redirect with a fresh INVITE to the dial-out port: `pjsua_safe::endpoint: incoming SIP call` fired (`Endpoint::poll_incoming_call` picked it up), `CardPool::handle_outbound_request` dispatched `ModuleCmd::Dial`, and a real `ATD+919789063708` reached the modem and was **accepted** — **T023's CS dial dispatch mechanism confirmed working on real hardware**, the first time this path has ever been exercised against a physical modem (T017f only got as far as opening the serial port, never reached a real `ATD`).
  4. The call then failed at the audio-bridging step: `outbound: failed to accept SIP leg after dial succeeded slot=0 error=media port error: pjsua_set_snd_dev returned 420002`. Root cause: `/proc/asound/cards` never showed an ALSA device for this modem at any point in the session, including after the `AT+QPCMV=1,2` ("voice audio routed to USB") command discovery already sends — `has_audio_capability` (`discovery.rs`) is deliberately a **static** per-model property (`2c7c:0125` = "EC20" = `true`), documented as intentionally not a live check, so the code did exactly what it's designed to do; this specific unit's USB audio composite interface simply never enumerated as an ALSA card in this environment during this session (likely needs a USB re-enumeration/reboot after `AT+QPCMV`, not just the AT command alone — not investigated further given time). **Also confirms today's earlier fix is live and correct**: the `ModuleCmd::Hangup`/`record_call_end` fix (fifth code review, finding #2) fired exactly as designed — `outbound: hanging up after SIP-side accept failed post-dial` — cleanly hanging up the real, already-placed carrier call rather than leaving it dangling.
  ...
  **Verdict**: the outbound-calling feature's own logic — registrar redirect, destination extraction, line claiming, `ModuleCmd::Dial`/`AT` dispatch, and graceful failure handling when the SIP-side accept fails post-dial — is now confirmed correct against real signaling and a real carrier dial attempt, for both the PBX/CS path (T023) and the SIP-server-mode phone path (T033). What remains unverified is a **fully audio-bridged** two-way call, blocked by this specific hardware unit's ALSA enumeration in this environment, not by anything in this codebase. Config and the live container were restored to their pre-test state (`[vowifi].enabled = true`, `[outbound]` absent) once testing finished; VoWiFi confirmed healthy again afterward.
