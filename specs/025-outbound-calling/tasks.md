...
- [X] T050e Mark the outbound-calling item complete in `docs/todo.md` — **DONE 2026-09-04**: CS (T023), VoWiFi (T072), and PC/SC (T073) were already audio-verified on real hardware; VoLTE was the last unverified path and is now confirmed live end to end (below). `docs/todo.md` item 2 checked off.
  ...
  **2026-09-04, second pass — actual bug found and fixed.** With `[volte].bridge_inbound=true`, `orchestrate_volte::start_multiline` already spawns `volte-bridge`, which reuses VoWiFi's exact `run_telephony_side` (registrar, `run_outbound_listener`, PBX-trunk UAS handling — all of it) and VoLTE's carrier-agent process is the same `ims::agent::serve_inbound` Agent A code VoWiFi uses. So the dispatcher was already shared; nothing needed building. The real bug: `RuntimeLine` (what `try_place_on_line` iterates over) had no `status_port` field, so every VoLTE `PlaceCall` attempt connected to VoWiFi's fixed `AGENT_A_STATUS_PORT` (5071) instead of the line's real per-line-derived port (5076/5080/5084/…). **Fixed**: added `RuntimeLine.status_port`, threaded through the VoWiFi and VoLTE (`BridgeLine::to_runtime_line`, new) construction sites and `try_place_on_line`/`print_status`; new regression tests in both `vowifi/mod.rs` and `volte/bridge.rs`; `make format`/`make lint`/`make test` all pass. PR: https://github.com/selvakn/gsm-sip-bridge/pull/72.

  **2026-09-04, third pass — a second, unrelated bug, then full live pass.** Re-running the live test hit "IMS PDN attached but no default route" (`routed=false`). Root-caused via a raw `AT+QNETDEVCTL?` probe: `volte::pdn::bring_up` only issues the rebind command (`AT+QNETDEVCTL=1,<cid>,1` — what actually gives the host interface fresh carrier) when the modem doesn't already report that context bound; an earlier session's `volte-register` run left context 3 marked bound without a clean detach, so the rebind was silently skipped on every subsequent attach/retry. Not a code bug — `gsm-sip-bridge volte-pdn --action down` clears the stale binding (confirmed `AT+QNETDEVCTL?` → `0,0,0,0` before and after). With that cleared, a full re-run on the local Vodafone/EC200U rig (`volte-bridge --modem` diagnostic path, `[sip_server]` mode) succeeded completely: real IMS REGISTER (`200 OK`), real outbound INVITE (100→183→180→200 OK), real bidirectional audio (`siptest`: `direction: both ways`, `success: true`), AMR-WB↔L16 transcoding both directions, corroborated by the daemon's own `call media verdict media="both-ways" carrier_rx=843 pbx_rx=921 ... outcome="answered"` and metrics (`gsm_sip_bridge_outbound_attempts_total{outcome="placed"} 1`, `gsm_sip_bridge_calls_total{status="answered",transport="volte"} 1`). Config/image reverted; VoWiFi confirmed `Registered`/`gm_connection: up` again afterward. See `docs/todo.md` item 2 for the closed-out summary.
