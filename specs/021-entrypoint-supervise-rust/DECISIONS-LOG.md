...
## 2026-07-27 (yet later) — a 13th bug: supervision loops could respawn processes that escape the shutdown plan entirely

Greptile's next pass found a different, architecturally deeper issue: `orchestrate::run`'s shutdown sequence takes a single snapshot of `StartedState` via `build_shutdown_plan`, then `execute_shutdown_plan` signals exactly the handles that were in it. But every per-process supervision loop in `orchestrate.rs`/`orchestrate_volte.rs` runs forever on its own background thread with zero awareness that shutdown has begun — if one of their processes exits around that time (including *because* the shutdown plan's own `KillChild` step just signaled it — a very plausible trigger, not a contrived edge case), the loop just spawns a replacement and registers it into `StartedState`, entirely outside the plan that already ran.

My first framing of the consequence (in the Greptile reply and this log's initial version of this entry) was more alarming than accurate, and worth correcting here: I initially claimed a process still alive in a netns when `DeleteNetns` runs would leak that namespace *on the host, indefinitely*. That overstates it. This container's compose config sets `network_mode: host` but not `pid: host` (checked `docker/docker-compose.yml` directly to confirm) — so its mount, PID, IPC, and UTS namespaces are all private to the container, same as any ordinary container; only the network namespace is shared with the host. A stray orphan spawned by a buggy supervision loop is therefore still scoped to the *same* PID namespace as everything else in the container — when `orchestrate::run`'s own process (PID 1 inside that namespace) finally exits at the end of `execute_shutdown_plan`, the kernel forcibly kills every remaining process in that namespace, orphan included, and tears down the container's private mount namespace along with whatever `/var/run/netns/*` bind-mount references lived in it. Nothing survives past the container's own lifetime. The **real** risk is narrower but still genuine: a `DeleteNetns` or `RunInNetns` step executing *while* a stray orphan is still alive inside that same namespace, during the shutdown sequence itself — `ip netns del` failing outright because something's still running in it, or a modem-cleanup command racing an orphan that still holds the serial port. A transient correctness problem during teardown, not a permanent host-level resource leak. Posted a follow-up correction on the Greptile thread; the fix itself doesn't change either way.

Fixed with a shared `shutting_down: Arc<AtomicBool>`, created once in `orchestrate::run`, threaded (the same `Arc::clone`-into-thread-closure pattern already used for `runner`/`started` throughout this module) into every loop that could spawn a new long-lived process:
- the circuit-switched daemon's own loop
- the shared `vowifi-sip-agent` loop
- the per-line `vowifi-usim-bridge` holder loop
- the per-line `vowifi-ims-agent` loop (inside `start_line_tail`)
- **both tunnel engines' steady-state loops** — the least obvious of the set, since these don't have a `loop { spawn }` shape of their own; they call `tick_steady_state`, whose `Recovered` outcome triggers `engine.restart_process()` *internally*, spawning a fresh charon/swu-dialer independent of any `KillChild` step ever running. Missing this one would have left the fix incomplete for exactly the scenario most likely to matter (a line's own tunnel engine dying and self-recovering right as shutdown begins).
- all three VoLTE loops (carrier-agent, bridge, legacy registration)

(Double-checked whether host-visible `ip netns list` during this investigation contradicted anything — it didn't: this container's own `/var/run/netns` lives in its own mount namespace, separate from the host shell's, even under `network_mode: host` (only the network namespace is shared, not the mount namespace) — so the host shell never seeing these namespaces is expected, not a finding. No further action needed there.)

Set to `true` immediately after `wait_for_signal()` returns, *before* `build_shutdown_plan` takes its snapshot — not after — so that any loop reading `false` and racing to spawn+register must complete that race against our own subsequent `started.lock()` call for the snapshot, which the Mutex serializes; the only genuinely residual window is a nanosecond-scale TOCTOU between a loop's own flag-check and its immediately-following spawn call, categorically different from the multi-second-to-30-second window this had before (steady-state ticks every 30s; several loops sleep 5-15s between iterations).

**No unit test added** — `orchestrate.rs`/`orchestrate_volte.rs` still have no test module at all (the same gap the T049 FR-009 audit already documented; testing this specific fix properly would need the same pure-decision-function extraction recommended there, which is a real, scoped follow-up, not something to rush alongside a live bug fix). Verified live instead, deliberately adversarially: killed a running `vowifi-ims-agent` process and sent the container `SIGTERM` back-to-back with only a 50ms gap between the two commands, specifically trying to land inside the race window — clean exit (code 0), no leftover host processes, no leaked netns. Given the fix closes the window down to nanoseconds, reliably *forcing* a live reproduction of the original bug is no longer really possible by design (that's the point), so this live test is necessarily a "didn't break anything, and didn't reproduce a failure under deliberate adversarial timing" check rather than a positive repro-then-fix-then-repro-again cycle like the other bugs in this log.

656 tests, `make lint` clean. Committed (`cc50275`), pushed, replied to Greptile's comment, re-triggered the review. **Thirteen real bugs found and fixed across this whole feature now.**

---

## 2026-07-27 (still later) — a 14th bug: the shutdown flag itself needed to be atomic with recovery, not just checked

Greptile's next pass on the same area found that the previous fix's `AtomicBool` closed the common case but not the one that matters most: `tick_steady_state`'s `Recovered` outcome calls `engine.restart_process()` *internally*, and — because of earlier fixes in this same session (the vici-socket-warmup `sleep(2)`, the reap-before-respawn `wait()`) — that whole recovery choreography now takes multiple real seconds to complete. A loop that read the flag as `false`, then started that multi-second sequence, could finish it and register the new handle into `StartedState` well *after* shutdown had already set the flag and taken its snapshot. The escape window was genuinely seconds wide, not the near-instant gap a bare check-then-spawn suggests — a sharper, more accurate version of the same finding, and worth taking seriously rather than assuming the first fix's flag check was "close enough."

Replaced `Arc<AtomicBool>` with `Arc<RwLock<bool>>` throughout: every supervision loop now takes a **read guard across the entire "maybe spawn, maybe register the handle" critical section** — for both tunnel engines' steady-state loops, that means holding the guard across the *whole* `tick_steady_state` call, not just checking before it. Shutdown takes a **write guard**, which blocks until every currently-held read guard is released, before setting the flag and taking the `StartedState` snapshot. This is the standard reader-writer pattern for exactly this shape of problem: many concurrent "maybe do a slow thing and register it" operations, one operation that must wait for all of them to either finish or not-yet-start before proceeding. Unlike the bare flag, this guarantees — not just makes likely — that no recovery is still in flight at the moment the snapshot is taken.

**Live-verification of this specific refinement was cut short by real hardware trouble, not a code problem**: partway through testing, the physical SIM went into a CME-ERROR-10 ("not inserted") state and stayed there through several restart attempts — most likely provoked by this session's own repeated rapid `kill -9 charon` / container-restart cycles across the last several fixes, a known failure mode for this class of hardware (see the project's own memory on SIM-drop recovery from a prior incident). Deliberately chose not to attempt an ad-hoc `AT+CFUN` power-cycle outside any tested code path to try to force a recovery — that felt like exactly the kind of unsupervised, higher-risk intervention worth stopping and flagging rather than pushing through. Confidence in this specific fix therefore rests on the `RwLock`'s well-defined, standard-library-guaranteed blocking semantics plus the full test suite passing, not a fresh live repro-then-fix-then-repro cycle like most of the other bugs in this log. **Recommend the user check on the physical modem/SIM seating** — a hardware-level condition needing physical access, well outside anything this session can further test or fix remotely.

656 tests, `make lint` clean. Committed (`74513e0`), pushed, replied to Greptile's comment (with the corrected, more precise diagnosis), re-triggered the review. **Fourteen real bugs found and fixed across this whole feature now** — and, notably, the last three all trace back to the same shutdown-snapshot design gap, found and progressively sharpened across three separate Greptile passes rather than caught whole in one review. Worth remembering for next time: a "fixed" reply to a review finding is worth re-reading skeptically when the reviewer comes back with a *more specific* version of the same concern, rather than assuming round one already covered it.
...
**Re-review after the 15th-bug fix came back 5/5, "appears safe to merge," no new findings.** This closes the shutdown-snapshot design gap after four consecutive rounds on it. The physical SIM, whose CME-ERROR-10 condition cut short live-verification of the RwLock refinement two entries back, had also fully recovered on its own by this point — confirmed the container healthy with a real established tunnel, not just a passing healthcheck.

## Where PR #14 stands now

**Fifteen real bugs found and fixed across this entire feature**, spanning the full session: two from live-hardware testing against the real EC20 + Airtel modem (`extract_latest_pcscf`'s log-prefix bug, the missing `recreate_interface` wiring), one from a proactive FR-009 comment-to-test audit turned live repro (the missing vici-socket-warmup sleep), and the remaining twelve from Greptile — most of them clustered around the same underlying theme of `CommandRunner` handle lifecycle and shutdown-snapshot correctness, each verified independently against the real code before being accepted or fixed, never taken on faith. None were caught by the 656-test unit suite alone; each is a real-runner-timing or real-hardware-behavior detail a mock or a fixture couldn't reproduce.

All 5 phases of the strangler plan are done, `docker/entrypoint.sh` is a 28-line shim, both VoWiFi (strongswan) and VoLTE (bridge_inbound) are live-validated end-to-end on the real modem including cold start, warm restart, forced degraded-state recovery, and clean shutdown, Phase 7 polish (T047-T052) is complete, and Greptile is at 5/5 as of the latest push. Recommend merging as-is. Remaining, explicitly non-blocking, documented follow-ups: a VoLTE-aware `healthcheck.sh` (pre-existing gap), test coverage for the handful of still-untested `orchestrate.rs`/`orchestrate_volte.rs` invariants noted in the T049 audit (would need a pure-decision-function extraction first), swu-engine and physical-SIM-unplug live scenarios (need a second SIM/profile or hands on the hardware, neither available this session).
