...
**Why this priority**: *Demoted from P1 to P3 after the Gate G1 spike* (see `plan.md`, `research.md` R2). Investigation against the live carrier established that **no automatic mechanism yields a P-CSCF on this carrier and modem**: DHCPv6 replies but returns no RFC 3319 SIP-server options, the Router Advertisement carries only prefix and MTU, and DNS is unusable because the only resolvers offered are null. Automatic discovery therefore cannot be a prerequisite for anything. Its remaining value is **diagnostic** — running the probes and reporting their results is what makes a future firmware, carrier, or SIM change discoverable — so it is worth building, but last.
